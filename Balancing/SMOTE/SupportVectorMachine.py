...
print("[SVM WITH SMOTE] Searching best params with GridSearchCV")
...
print(f"[SVM WITH SMOTE] Best SVM model with params: {svm_model_gridcv.best_params_} and score: {svm_model_gridcv.best_score_:.3f}")

# save model
svm_model = svm_model_gridcv.best_estimator_
save_estimator(svm_model, "SVM_SMOTE.joblib")
print("[SVM WITH SMOTE] SVM_SMOTE model saved")

# predict
score = balanced_model_predict(model=svm_model, name="SVM_SMOTE", test_data=data_test, test_labels=labels_test)
print("[SVM WITH SMOTE] Balanced accuracy score:", score)

# get important features per class
get_features_importances_SVM(svm=svm_model, selected_features=selected_features, name='SVM_SMOTE')
